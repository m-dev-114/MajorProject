...
<div style='background:#1a1a2e;border:1px solid #333355;border-radius:10px;
    ...
    <div style='font-size:0.75rem;color:#888;line-height:1.5;'>{desc}</div>
        ...
        if 'Assignee' in df.columns:
            ...
            # Pure pandas styling — no matplotlib needed
            def style_row(row):
                styles = []
                for col in row.index:
                    ...
                    try:
                        ...
                        continue
                    if col == 'Health Score':
                        c = '#06d6a0' if v >= 60 else ('#ffd166' if v >= 40 else '#ff4d6d')
                    else:
                        c = '#ff4d6d' if v > 60 else ('#ffd166' if v > 30 else '#06d6a0')
                    styles.append(f'background-color:{c}22;color:#e0e0f0')
                return styles
            styled = heat_df.style.apply(style_row, axis=1).format("{:.1f}", na_rep="—")
...
<div style='background:#1a1a2e;border:1px solid #333355;border-radius:8px;padding:0.8rem;text-align:center;'>
  ...
  {''.join([f"<span style='background:{color}22;color:{color};padding:2px 8px;border-radius:3px;font-size:0.75rem;margin:2px;display:inline-block;'>{m}</span>" for m in members])}
                    ...
                    def color_cluster_row(row):
                        ...
                        return styles
                    st.dataframe(
                        display_df.style.apply(color_cluster_row, axis=1).format("{:.2f}", na_rep="—"),
                        use_container_width=True
                    )
        except Exception as e:
